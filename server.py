...
def rules(target):
    ...
    try:
        ...
        return True
    # Subdomain.
    for ah in ACCEPT_IPS:
        sd = target[:tp].replace(ah,"")
        if (ah.find('.')==0 and target[:tp]==sd+ah):
            return True
    return False
    ...
    def run(self):
        ...
        try:
            ...
            if action == AcceptClient.ACTION_CREATE:
                ...
                if rules(target)==False:
                    self.log('Target is Denied', Logger.LOG_WARN)
                    self.socket.sendall('HTTP/1.1 403 Access Denied\r\nServer: TOHTServer\r\n\r\n')
                    return
